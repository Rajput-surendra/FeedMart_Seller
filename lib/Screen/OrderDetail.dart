...
class StateOrder extends State<OrderDetail> with TickerProviderStateMixin {
  ...
  getPermission() async{
    permission = (await getPrefrence(Permission))!;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    customPermission  = prefs.getString('customerPermission');
    print("1111111111111111${permission}");
    print("1111111111111111${customPermission}");
  }
  ...
  checkPermission()async{
    print("working here");
    var headers = {
      'Cookie': 'ci_session=8ae8206daec604a14f7d0bd9d2d4eb5db4ffbcfb'
    };
    var request = http.Request('GET', Uri.parse('${baseUrl}get_user/${CUR_USERID}'));
    print("checking api here"+ " " + "${baseUrl}get_user/${CUR_USERID}");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    print("yes here ${response.statusCode}");
    if (response.statusCode == 200) {
      var finalResult = await response.stream.bytesToString();
      final jsonResponse = json.decode(finalResult);
      print("check result ${jsonResponse}");
      finalPermission = jsonResponse['data']['permissions']['customer_privacy'];

      print("checking value  here ${finalPermission}");
    }
    else {
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
                ...
                child: Text(
                  permission == "0" ?
                    "${capitalize(tempList[0].name!.substring(1,3))}....." : capitalize(tempList[0].name .toString()),
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                        permission == "0" ?
                        "${capitalize(tempList[0].mobile!.substring(1,3))}....." : capitalize(tempList[0].mobile .toString()),
                        // " " + tempList[0].mobile!,
                        style: TextStyle(
                            color: primary,
